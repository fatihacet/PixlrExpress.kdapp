...
class PixlrAppView extends JView
  ...
  buildIframeSrc : (useEscape, isSplashView) -> 
    ...
    """#{PixlrSettings.src}/?#{img}&title=#{PixlrSettings.imageName}&target=#{PixlrSettings.targetPath}#{amp}meta=#{PixlrSettings.savePath}&icon=#{PixlrSettings.saveIcon}&referer=Koding&type=#{PixlrSettings.fileExt}&key=#{@mem}"""
